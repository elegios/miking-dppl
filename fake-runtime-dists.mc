...
-- Base interface
lang FakeRuntimeDistBase
  syn RDist a =

  sem sample : all a. RDist a -> a

  sem expectation : all a. RDist a -> a

  sem logObserve : all a. RDist a -> (a -> Float)
end
...
lang FakeRuntimeDistElementary = FakeRuntimeDistBase
  syn RDist a =
...
lang FakeRuntimeDistEmpirical = FakeRuntimeDistBase
  syn RDist a =
  | DistEmpirical {
  ...
  | extra ->
    ...
    printLn (join ["constructDistEmpirical took ", float2string (subf endTime startTime), "ms"]);
...
lang FakeRuntimeDist =
  FakeRuntimeDistElementary +
  FakeRuntimeDistEmpirical
end
