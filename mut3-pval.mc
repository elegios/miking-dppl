...
lang MutPValExp3 = PValInterfaceExp
  ...
  sem p_map st f = | a ->
    p_subMap_ st (lam st. lam a. (st, f a)) a
  ...
  --         then theseHasHere (deref branches)
  --         else theseHasThere (deref branches) in
  ...
  --     then optionMapOrElse (lam. never) (lam x. x.ist) (theseGetHere (deref branches))
  --     else optionMapOrElse (lam. never) (lam x. x.ist) (theseGetThere (deref branches)) in
  --   let st =
  ...
  sem p_match st store ist a pick = | build ->
    ...
    let update = lam st.
      let updateCurrentSubGraph = lam st.
        (deref updateRef) st;
      ...
      if eqi st.id (deref a.changeId) then
        ...
        if eqb (optionIsSome prevPicked) (optionIsSome newPicked) then
          ...
          updateCurrentSubGraph {st with pretendFresh = true}
        else
