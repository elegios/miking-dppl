...
--   = lam p.
--     { sample = lam. exponentialSample p
--     , logObserve = lam v. exponentialLogPdf p v
--     }
...
type PDist a = use FakeRuntimeDist in RDist a
let p_sample : all a. PDist a -> a = use FakeRuntimeDist in sample
let p_logObserve : all a. PDist a -> a -> Float = use FakeRuntimeDist in logObserve

let p_bernoulli : Float -> PDist Bool = use FakeRuntimeDist in lam p. DistBernoulli {p = p}
let p_exponential : Float -> PDist Float = use FakeRuntimeDist in lam rate. DistExponential {rate = rate}
let p_uniformDiscrete : Int -> Int -> PDist Int = use FakeRuntimeDist in lam min. lam max. DistUniformDiscrete {a = min, b = max}
let p_gaussian : Float -> Float -> PDist Float = use FakeRuntimeDist in lam mu. lam sigma. DistGaussian {mu = mu, sigma = sigma}
let p_beta : Float -> Float -> PDist Float = use FakeRuntimeDist in lam a. lam b. DistBeta {a = a, b = b}

-- === Finally tagless representation of piece-wise static PVal models ===

lang PValInterfaceExp
  syn PVal a =
  syn PValState st =
...
lang PruneBasics = PValInterfaceExp
  syn Prune a =
